...
**Easy Mic** 是一款面向 Unity 的高性能、低延迟音频录制插件。它提供对原始麦克风数据的直接访问，并引入可编程音频处理流水线，帮助开发者用统一的 Unity API 构建实时录音、播放、语音识别和语音交互工作流。
...
| 能力 | 本仓库是否包含 | 说明 |
| --- | --- | --- |
| 麦克风采集 | 包含 | 低延迟原生采集，可访问原始音频缓冲区。 |
| 音频处理流水线 | 包含 | 可组合录制、降混、音量门、回环监听等处理器。 |
| Unity 组件 | 包含 | 提供录音、播放、ASR 编排、TTS 播放等场景组件。 |
| Sherpa ONNX 集成 | 可选 | 需要额外安装 [`com.eitan.sherpa-onnx-unity`](https://github.com/EitanWong/com.eitan.sherpa-onnx-unity)。 |
| AEC / AGC / ANS | 不包含 | 仅在付费扩展包 EasyMic APM 中提供。 |
...
*   **🧩 实用的内置处理器库**: 提供录制、降混、音量门、回环监听等常用处理器，开箱即用。
*   **🔌 可扩展架构**: 支持自定义处理器、第三方集成，以及独立扩展包接入。
...
对于需要语音通信清理能力的生产应用，请考虑 **EasyMic Audio Processing Module (APM)**。

**重要说明：** EasyMic APM 是单独提供的付费扩展包。本开源仓库不包含 AEC、AGC、ANS 的实现代码、二进制文件、示例或授权。

*   **🚫 AEC (声学回声消除)**: 消除声学回声，实现水晶般清晰的语音通信
*   **🔇 ANS (自动噪声抑制)**: 在保持语音质量的同时去除背景噪声
*   **📊 AGC (自动增益控制)**: 自动维护一致的音频电平

**推荐用于 AI 数字人与虚拟主播**: 解决 Unity 对话式 AI 应用中“系统播放声音再次被麦克风采集”的常见回声问题。

💰 EasyMic APM 为付费扩展包，使用前需联系作者购买许可证。<br>
📧 联系方式：[unease-equity-5c@icloud.com](mailto:unease-equity-5c@icloud.com)<br>
🛒 第三方商店即将上线，便于购买与授权。
...
- 本仓库不包含回声消除、增益控制和噪声抑制能力。如需这些能力，请使用单独付费的 EasyMic APM 扩展包。
...
- 💎 **EasyMic APM**: AEC、AGC、ANS 付费扩展包，请联系 [unease-equity-5c@icloud.com](mailto:unease-equity-5c@icloud.com)
