...
namespace Eitan.EasyMic.Runtime
{
    ...
    public sealed class AudioMixer : IDisposable
    {
        ...
        public void RenderAdditive(Span<float> buffer, int systemChannels, int systemSampleRate)
        {
            ...
            if (anySolo && !Solo && !HasSoloInChildren())
            {
                return;
            }
        ...
        public void AddSource(PlaybackAudioSource source)
        {
            ...
            while (true)
            {
                ...
                next[^1] = source;
                if (System.Threading.Interlocked.CompareExchange(ref _sourcesSnap, next, cur) == cur)
                {
                    break;
                }
        ...
        public void AddMixer(AudioMixer mixer)
        {
            ...
            while (true)
            {
                ...
                Array.Copy(cur, next, cur.Length);
                next[^1] = mixer;
                if (System.Threading.Interlocked.CompareExchange(ref _mixersSnap, next, cur) == cur)
                {
                    break;
                }
        ...
        public bool HasSoloRecursive()
        {
            ...
            for (int i = 0; i < mixers.Length; i++)
            {
                if (mixers[i]?.HasSoloRecursive() == true)
                {
                    return true;
                }
            }
