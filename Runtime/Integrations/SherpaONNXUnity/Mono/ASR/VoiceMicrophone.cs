...
namespace Eitan.EasyMic.Runtime.Integration.SherpaONNXUnity.Mono.ASR
{
    ...
    /// </summary>
    [AddComponentMenu("Audio/EasyMic/Input/Voice Microphone")]
...
{
    [AddComponentMenu("Audio/EasyMic/Input/Voice Microphone")]
    public class VoiceMicrophone : EasyMicrophone
    {
        ...
        public AutomaticSpeechRecognitionConfiguration AsrConfig =>
            _asrConfig ??= AutomaticSpeechRecognitionConfiguration.CreateDefault();

        public bool IsSpeaking => false;
        public bool IsVoiceActivity => false;
        public bool RequiresStreaming => AsrConfig.RecognitionMode != RecognitionMode.KeywordSpottingOnly;
        public bool RequiresOffline => AsrConfig.RecognitionMode != RecognitionMode.Streaming;
        public bool RequiresVad => AsrConfig.RecognitionMode != RecognitionMode.Streaming;
        public bool RequiresPunctuation => AsrConfig.EnablePunctuation;
        public bool RequiresKeywordSpotting => AsrConfig.ActiveKeywordOptions.IsEnabled;
        public IReadOnlyList<AutomaticSpeechRecognitionConfiguration.ASRPreset> SupportedPresets => AsrConfig.SupportedPresets;
        public string ActivePresetId => AsrConfig.ActivePresetId;
        ...
        public bool ApplyConfiguration(AutomaticSpeechRecognitionConfiguration configuration)
        {
            _asrConfig = configuration?.Clone() ?? AutomaticSpeechRecognitionConfiguration.CreateDefault();
            return true;
        }

        public void ConfigureTurnDetection(TurnDetectionOptions options)
        {
            var config = AsrConfig.Clone();
            var preset = config.GetActivePreset();
            preset.TurnDetectionOptions = options.EnsureValid();
        ...
        private static void LogMissingDependencyOnce()
        {
            ...
            Debug.LogWarning(
                "[EasyMic] VoiceMicrophone is running in compatibility mode because com.eitan.sherpa-onnx-unity is not installed. " +
                "The component reference is preserved, but ASR features are disabled.");
        }
    }
}
#endif
