    ...
    public static class AIChatSetupLocalization
    {
        ...
        private static readonly IReadOnlyDictionary<AIChatSetupTextKey, string> English =
            new Dictionary<AIChatSetupTextKey, string>
            {
                ...
                { AIChatSetupTextKey.DebugMode, "Developer Debug Mode" },
                { AIChatSetupTextKey.AudioSetupStep, "Audio and models" },
                { AIChatSetupTextKey.AudioSetupHelp, "Select VoiceMicrophone: choose an input device, ASR model and enable APM/AEC for speakers. In Provider Setup select local or remote speech. For local speech, select SpeechSynthesizer, choose the model, keep automatic volume balance enabled, and start at volume 1. Use the model's native sample rate." },
                { AIChatSetupTextKey.FirstConversationStep, "Test your first conversation" },
                { AIChatSetupTextKey.FirstConversationHelp, "Save the scene, enter Play Mode and allow microphone access. Wait for model loading to finish. First listen without speaking: the assistant should finish its greeting. Then speak during playback: your voice should interrupt it. Check normal and quiet phrases. To investigate a failure, enable Developer Debug Mode and press F12; turn it off after testing." },
                { AIChatSetupTextKey.DebugModeHelp, "Enables F12 pipeline diagnostics and component logs. Transcript logging and TTS payload diagnostics remain separate opt-ins in the Controller inspector." },
                { AIChatSetupTextKey.NormalizeLocalTts, "Automatic volume balance" },
                { AIChatSetupTextKey.LocalTtsVolume, "Playback volume" },
                { AIChatSetupTextKey.LocalTtsAudioHelp, "Bounded RMS levelling and -1 dBFS sample-peak protection run before playback and AEC. Start at volume 1; 0 mutes and 2 adds up to 6 dB. Select SpeechSynthesizer to choose the model." },
        ...
        private static readonly IReadOnlyDictionary<AIChatSetupTextKey, string> ChineseSimplified =
            new Dictionary<AIChatSetupTextKey, string>
            {
                ...
                { AIChatSetupTextKey.DebugMode, "开发者调试模式" },
                { AIChatSetupTextKey.AudioSetupStep, "配置音频与模型" },
                { AIChatSetupTextKey.AudioSetupHelp, "选择 VoiceMicrophone：确认输入设备、ASR 模型；外放时启用 APM/AEC。在 Provider 设置中选择本地或远程语音。本地语音选择 SpeechSynthesizer 设置模型，保持自动均衡开启，音量从 1 开始，并使用模型原生采样率。" },
                { AIChatSetupTextKey.FirstConversationStep, "验证第一次对话" },
                { AIChatSetupTextKey.FirstConversationHelp, "保存场景后进入 Play Mode，允许麦克风访问并等待模型加载完成。先保持安静，确认助手能完整播完问候；再在播放时说话，确认能及时打断。试听正常与较轻的句子。遇到问题时开启开发者调试模式，按 F12 查看流水线；排查后关闭调试。" },
                { AIChatSetupTextKey.DebugModeHelp, "启用 F12 流水线诊断及组件日志。逐字转写日志和 TTS 音频诊断仍需在 Controller Inspector 中单独开启。" },
                { AIChatSetupTextKey.NormalizeLocalTts, "自动均衡音量" },
                { AIChatSetupTextKey.LocalTtsVolume, "播放音量" },
                { AIChatSetupTextKey.LocalTtsAudioHelp, "在播放与 AEC 参考信号之前进行受限 RMS 均衡及 -1 dBFS 采样峰值保护。音量从 1 开始；0 静音，2 最多增加 6 dB。选择 SpeechSynthesizer 组件设置模型。" },
